...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `saiu @${num.split('@')[0]} filho da putakkkk`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5511954245328@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`falha ao transformar ${tipe} para figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'sem porno no grupo'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
				...
				case 'belle2':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'gostosinha'})
					break
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/dPUVFF6.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*_comandos basicos para criar o bot:_*\n\n*pkg upgrade && pkg update*\n*pkg install git*\n*git clone (link da git)*\n*cd (repositório)*\n*bash install.sh*\n*npm start*\n\n*pacoca hackiagens'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'me deixou pensativo rabudinha'})
					break
				case 'akeno':
					meme = await kagApi.memes()
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'tapoha'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia e vai toma no cu'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'boa tarde apenas'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'vai dormir desgraca boa noite'})
					break
				case 'belle':
					memein = await kagApi.memeindo()
				...
				case 'nsfwloli':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'i ala'})
					break
				case 'reislin':
					memein = await kagApi.memeindo()
				...
				case 'memeindo':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'tapoha o mlk e fluente em indoenisdiasia'})
					break
				case 'setprefix':
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'c e viado e'})
					})
					break
				case 'nsfwloli':
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('coloca oq se vai procurar')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'bota o nome do corno ae', text, {quoted: mek})
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('qual tipo é?')
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('só funciona se voce for o dono do bot')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('excluido todos os chats :)')
					break
				case 'ts':
        ...
        case 'promote':
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`@${mentioned[0].split('@')[0]} foi rebaixado mt burro`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('coloque o numero de quem voce quer adicionar, sem espaço ou "+"')
					if (args[0].startsWith('08')) return reply('coloque o código do país')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('falha ao adicionar, talvez esteja para só contatos adicionar')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKKKKKKKKKKKKzuo pacarai @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('nao funciona com figurinhas que se mexem ❌')
					reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar o modo simi, 0 para desativar')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
